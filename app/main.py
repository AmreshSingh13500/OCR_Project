...
[SUMMARY]  FastAPI application entrypoint, started via `uvicorn app.main:app`. Defines
           ...
           and loads the CLIP model on startup. This file has no single owning subtask
           beyond T1.1.x scaffolding — later subtasks extend it: router mounting (T1.2.3
           process endpoint, T1.2.4 health endpoint) and model loading (T3.1.1 CLIP,
           T3.2.1 PaddleOCR) are added in place per CODING_RULES.md Rule 3.
[PLAN]     IMPLEMENTATION_PLAN.md §2 → T1.1.1, T1.1.3; §4 T3.1
